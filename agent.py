...
class Agent(object):
    ...
    def action_ensemble(self, observation, desired_goal):
        observation = torch.FloatTensor(observation).to(self.device)
        desired_goal = torch.FloatTensor(desired_goal).to(self.device)
        dist = self.actor(observation, desired_goal)
        return dist
    ...
    def get_values(self, observation, 
        ...
        val1, val2 = self.critic(observation, desired_goal, action)
        return utils.to_np(val1), utils.to_np(val2)

    def get_traget_values(self, observation, 
        desired_goal, action):
        observation = torch.FloatTensor(observation).to(self.device)
        desired_goal = torch.FloatTensor(desired_goal).to(self.device)
        action = torch.FloatTensor(action).to(self.device)
        val1, val2 = self.critic_target(observation, desired_goal, action)
        return utils.to_np(val1), utils.to_np(val2)
